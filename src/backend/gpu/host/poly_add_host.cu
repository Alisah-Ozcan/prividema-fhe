...
static int launch_poly_add(const Data64* a, const Data64* b, Data64* c,
...
int gpu_poly_add_mod(const uint64_t* a, const uint64_t* b, uint64_t* c,
                     uint64_t n, uint64_t q)
{
    return launch_poly_add(a, b, c, n, Modulus64(q), nullptr, nullptr);
}

int gpu_poly_add_mod_timed(const uint64_t* a, const uint64_t* b, uint64_t* c,
                            uint64_t n, uint64_t q, float* gpu_kernel_ms)
{
