...
static void cpu_poly_add_mod(const uint64_t* a, const uint64_t* b,
...
Test(gpu_poly_add, correctness_vs_cpu)
{
    ...
    for (uint64_t i = 0; i < n; i++)
        cr_assert_eq(gpu_c[i], cpu_c[i],
                     "Mismatch at index %llu: cpu=%llu  gpu=%llu",
                     (unsigned long long)i, (unsigned long long)cpu_c[i],
                     (unsigned long long)gpu_c[i]);
